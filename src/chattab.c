...
static HWND chat_window;

static TabItem *private_tabs;
static TabItem *channel_tabs;
static const TabItem *active_tab;
...
resize_active_tab(void)
{
	...
	list = active_tab->list_window;
	chat = active_tab->chat_window;
...
add_tab(const TabItem *item)
{
	...
	return SendMessage(tab_control, TCM_INSERTITEMA, INT_MAX, (intptr_t)&info);
}
...
get_tab_index(const TabItem *tab_item)
{
	...
	item_len = SendMessage(tab_control, TCM_GETITEMCOUNT, 0, 0);

	for (int i=0; i<item_len; ++i) {
		TCITEM info;

		info.mask = TCIF_PARAM;
		SendMessage(tab_control, TCM_GETITEM, (uintptr_t)i,
		    (intptr_t)&info);
...
{
	if (active_tab) {
		ShowWindow(active_tab->chat_window, 0);
		ShowWindow(active_tab->list_window, 0);
	}
	...
	active_tab = tab_item;
	resize_active_tab();
	MainWindow_set_active_tab(chat_window);
}
...
remove_tab(const TabItem *item)
{
	...
	info.mask = TCIF_PARAM;
	TabCtrl_GetItem(tab_control, index - (index > 0), &info);
...
chat_window_proc(HWND window, uint32_t msg, uintptr_t w_param, intptr_t l_param)
{
	...
	case WM_NOTIFY: {
		...
		if (tab_index < 0)
			return 0;
		info.mask = TCIF_PARAM;
		SendMessage(tab_control, TCM_GETITEM, (uintptr_t)tab_index,
		    (intptr_t)&info);
		focus_tab((TabItem *)info.lParam);
		return 0;
	}
